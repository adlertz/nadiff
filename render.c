...
draw_list(struct diff_array * da, struct window * list)
{
    ...
    for (unsigned i = 0; i < da->size; ++i) {
        ...
        if (strcmp(d->short_pre_img_name, d->short_post_img_name) == 0)
            ...
            char name[200];
            sprintf(name, "%s -> %s", d->short_pre_img_name, d->short_post_img_name);
